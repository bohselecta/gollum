...
type reqState struct{ ctx context.Context; req *GenRequest; ch chan Token; trace *Trace; generated int; created time.Time; kv *KVBlock }
type Scheduler struct{ mu sync.Mutex; incoming, active []*reqState; closed bool; backend KernelOps; maxBatch int; stepInterval time.Duration; pc *PromptCache; pgr *KVPager; pfx *PrefixCache }
func NewScheduler(b KernelOps)*Scheduler{ return &Scheduler{backend:b,maxBatch:32,stepInterval:25*time.Millisecond, pc: NewPromptCache(512), pgr: NewKVPager(), pfx: NewPrefixCache()} }
...
func (s *Scheduler) tick(){
    ...
    if len(prefill)>0 {
        ...
        for _,rs := range prefill {
            if ref, ok := s.pfx.Get(rs.req.Model, rs.req.Prompt); ok {
                if blk := s.pgr.ByID(ref.BlockID); blk != nil { s.pgr.Pin(blk); rs.kv = blk }
            }
            b.Prompts = append(b.Prompts, rs.req.Prompt)
        }
        _ = s.backend.Prefill(b)
        for _,rs := range prefill {
            if rs.trace.TTFTMs==0 { rs.trace.TTFTMs = time.Since(rs.created).Milliseconds() }
            s.mu.Lock(); s.active = append(s.active, rs); s.mu.Unlock()
        }
    }
    // Decode step
    ...
    for i, rs := range active {
        ...
        if rs.req.MaxTokens>0 && rs.generated>=rs.req.MaxTokens {
            ...
            if rs.kv == nil { rs.kv = s.pgr.Allocate(2048) }
            if rs.kv != nil { s.pfx.Set(rs.req.Model, rs.req.Prompt, KVRef{BlockID: rs.kv.ID, Tokens: rs.generated}); s.pgr.Unpin(rs.kv) }
            continue
        }
